...
For selected cities, the original declassified film scans (2-3 GB per frame) are downloaded from the USGS archive, placed on the map, and rendered as lossless PNG tiles stored in Cloudflare R2 (the repository holds the application, catalogs, manifests, and calibration records). Two editorial rules govern this pipeline: the film is never reshaped — archive corner coordinates only position a frame (rotation, uniform scale, mirroring); and a scene's overlay becomes publicly visible only after a human has verified its placement by eye (`"approved"` in `public/tiles/index.json`). Scenes with tiles awaiting verification appear in the interface as PENDING, without an overlay. Currently approved: Hama (1966, hand-aligned reference, the only building-level comparison — see `docs/DATA_PIPELINE.md` §5); further processed frames (Aleppo, Latakia, Idlib, Deir ez-Zor, Homs+Damascus, Raqqa region, the 1968 pre-Lake-Assad Euphrates strip) await by-eye alignment.
